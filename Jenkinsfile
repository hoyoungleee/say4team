    ...
    pipeline {
        ...
        environment {
            ...
            ECR_URL = "664418999523.dkr.ecr.ap-northeast-2.amazonaws.com"
        ...
        stages {
            ...
            stage('Deploy Changed Services to EKS Cluster') {
                when {
                    expression { env.CHANGED_SERVICES != "" }
                }
                steps {
                    script {
                        withAWS(region: "${REGION}", credentials: "aws-key") {
                            sh """
                            echo "[INFO] Kubeconfig 설정 중..."
                            aws eks update-kubeconfig --region ${REGION} --name eks-say4team

                            echo "[INFO] 변경된 서비스 목록: ${env.CHANGED_SERVICES}"

                            for service in ${env.CHANGED_SERVICES.replace(",", " ")}; do
                                echo "[INFO] 서비스 배포 중: \$service"
                                kubectl apply -f k8s/\$service/
                            done

                            echo "[INFO] 공통 Ingress 설정 적용 중..."
